...
public class DlgMarkingImageAssMedisIGD extends javax.swing.JDialog {
    ...
    public DlgMarkingImageAssMedisIGD(java.awt.Frame parent, boolean modal) {
        ...
        JButton btnAmbilHasil = new JButton("Ambil Hasil (Untuk Digambar)");
btnAmbilHasil.setToolTipText("Ambil gambar hasil USG pasien, jika tidak ada pakai template obgyn");
btnAmbilHasil.addActionListener(e -> popupPilihFotoLaluTampilkan(TNoRawat.getText().trim()));
panelGlass8.add(btnAmbilHasil);
//        setSize(885,674); 
    }
...
//        System.out.println(index);
  ...
  try {
    // Langsung tampilkan gambar template obstetri (obgyn.png)
    String imageUrl = "http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB() + "/" +
                      koneksiDB.HYBRIDWEB() + "/imagefreehand/masterimage/obgyn.png";

    System.out.println("URL Template USG: " + imageUrl); // Debugging
...
void imageAssesment(String url){
    try {
        System.out.println("[imageAssesment] load: " + url);
        java.awt.image.BufferedImage img = javax.imageio.ImageIO.read(new java.net.URL(url));
        if (img == null) throw new java.io.IOException("ImageIO.read -> null");
        PanelWall.setBackgroundImage(new javax.swing.ImageIcon(img));
        PanelWall.repaint();
    } catch (Exception ex) {
        System.out.println("[imageAssesment][ERR] " + ex.getMessage());
        try {
            String tmpl = buildHybridUrl("/imagefreehand/masterimage/obgyn.png");
            java.awt.image.BufferedImage def = javax.imageio.ImageIO.read(new java.net.URL(tmpl));
            PanelWall.setBackgroundImage(new javax.swing.ImageIcon(def));
            PanelWall.repaint();
        } catch (Exception ignore) {}
    }
}
private void onAmbilHasilClick() {
    final String noRawat = TNoRawat.getText().trim();
    if (noRawat.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Nomor rawat kosong / belum dipilih.");
        return;
    }

    try {
        String urlHasil = getPhotoUrlByNoRawat(noRawat);
        if (urlHasil != null) {
            System.out.println("Memuat hasil USG: " + urlHasil);
            imageAssesment(urlHasil); // tampilkan hasil asli untuk digambar
        } else {
            // fallback: pakai template obgyn
            String urlTemplate = buildHybridUrl("/imagefreehand/masterimage/obgyn.png");
            System.out.println("Tidak ada hasil. Pakai template: " + urlTemplate);
            imageAssesment(urlTemplate);
        }
    } catch (Exception ex) {
        System.out.println("Error ambil hasil: " + ex.getMessage());
        // fallback aman
        String urlTemplate = buildHybridUrl("/imagefreehand/masterimage/obgyn.png");
        imageAssesment(urlTemplate);
    }
...
private String getPhotoUrlByNoRawat(String noRawat) throws Exception {
    final String sql =
        "SELECT photo " +
        "FROM hasil_pemeriksaan_usg_gambar " +
        "WHERE no_rawat=? "; // sesuaikan nama kolom waktu jika berbeda
        ...
        try (ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                ...
                if (photo != null && !photo.isEmpty() && !photo.equals("-")) {
                    // rakit URL file hasil
                    return buildHybridUrl("/hasilpemeriksaanusg/" + photo);
                }
            }
        }
    }
    return null; // tidak ada hasil valid
}

private java.util.List<String> getPhotoList(String noRawat) {
    java.util.List<String> list = new java.util.ArrayList<>();
    final String sql = "SELECT photo FROM hasil_pemeriksaan_usg_gambar WHERE no_rawat=?";
    try (PreparedStatement ps = koneksi.prepareStatement(sql)) {
        ps.setString(1, noRawat);
        try (ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                String p = rs.getString("photo");
                if (p != null && !p.trim().isEmpty() && !"-".equals(p)) list.add(p.trim());
            }
        }
    } catch (Exception e) {
        System.out.println("[ERR] getPhotoList: " + e.getMessage());
    }
    return list;
}

private void popupPilihFotoLaluTampilkan(String noRawat) {
    java.util.List<String> photos = getPhotoList(noRawat);
    if (photos.isEmpty()) {
        imageAssesment(buildHybridUrl("/imagefreehand/masterimage/obgyn.png"));
        return;
    }
    ...
    String chosen = (String) javax.swing.JOptionPane.showInputDialog(
        this,
        "Pilih gambar hasil USG:",
        "Pilih Gambar",
        javax.swing.JOptionPane.QUESTION_MESSAGE,
        null,
        photos.toArray(new String[0]),
        photos.get(0)
    );

    if (chosen != null && !chosen.trim().isEmpty()) {
        String finalUrl = normalizeUrl(chosen);
        imageAssesment(finalUrl);            // <-- dipasang ke PanelWall
    }
}

private void pilihDanTampilkanFoto(String noRawat) {
    java.util.List<String> photos = getPhotoList(noRawat);
    if (photos.isEmpty()) {
        // fallback template
        imageAssesment(buildHybridUrl("/imagefreehand/masterimage/obgyn.png"));
    ...
    // jika cuma 1, langsung pakai
    if (photos.size() == 1) {
        imageAssesment(normalizeUrl(photos.get(0)));
        return;
    }
    ...
    if (chosen != null) imageAssesment(normalizeUrl(chosen));
}

// helper
private String buildHybridUrl(String path) {
    String host = koneksiDB.HOSTHYBRIDWEB();
    if ("localhost".equalsIgnoreCase(host) || "127.0.0.1".equals(host)) {
        host = "192.168.1.134"; // ganti IP server-mu
    }
    if (path == null) path = "/";
    String p = path.trim().replace("\\","/").replace(" ", "%20");
    if (!p.startsWith("/")) p = "/" + p;
    return "http://" + host + ":" + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + p;
}
...
private String normalizeUrl(String raw) {
    ...
    // kalau DB simpan nama file saja, default ke folder hasilpemeriksaanusg
    if (!u.startsWith("/")) u = "/hasilpemeriksaanusg/" + u;
    return buildHybridUrl(u);
}

//private String buildHybridUrl(String pathAfterContext) {
//    // pastikan pathAfterContext diawali '/'.
//    String base = "http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB()
//                  + "/" + koneksiDB.HYBRIDWEB();
//    return base + pathAfterContext;
//}
